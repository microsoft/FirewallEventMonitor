...
namespace ntl {
    ...
    std::tuple<double, double, double> SampledStandardDeviation(const BidirectionalIterator& _begin, const BidirectionalIterator& _end)
    {
    ...
    ///
    template <typename BidirectionalIterator>
    std::tuple<double, double, double> InterquartileRange(const BidirectionalIterator& _begin, const BidirectionalIterator& _end)
    {
        auto size = _end - _begin;
        if (size < 3) {
            return std::make_tuple(
        ...
        auto find_median = [] (const std::tuple<BidirectionalIterator, BidirectionalIterator>& _split) -> double {
            ...
            default:
                ntl::AlwaysFatalCondition(L"InterquartileRange internal error - returned iterators more than one apart [%Iu]", rhs - lhs);
            }
            return median_value;
        };
