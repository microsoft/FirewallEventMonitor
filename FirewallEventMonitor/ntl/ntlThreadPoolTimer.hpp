...
namespace ntl {
    namespace details
    {
        struct ThreadpoolTimerCallbackInfo {
            std::function<void(void)> callback;
            FILETIME timer_expiration = {0};
            unsigned long reoccuring_period = 0;

            ThreadpoolTimerCallbackInfo() = default;
            ~ThreadpoolTimerCallbackInfo() = default;
            // non-copyable
            ThreadpoolTimerCallbackInfo(const ThreadpoolTimerCallbackInfo&) = delete;
            ThreadpoolTimerCallbackInfo& operator=(const ThreadpoolTimerCallbackInfo&) = delete;

            explicit ThreadpoolTimerCallbackInfo(std::function<void(void)>&& _callback, long long _milliseconds) NOEXCEPT :
                callback(std::move(_callback))
            {
                using namespace ntl::Timer;
                timer_expiration = convert_msec_absolute_filetime(snap_system_time_as_msec() + _milliseconds);
            }
            explicit ThreadpoolTimerCallbackInfo(std::function<void(void)>&& _callback, long long _milliseconds, unsigned long _period) NOEXCEPT :
                callback(std::move(_callback)),
            ...
            ThreadpoolTimerCallbackInfo(ThreadpoolTimerCallbackInfo&& _callback_info) NOEXCEPT
            {
            ...
            void swap(ThreadpoolTimerCallbackInfo& _in) NOEXCEPT
            {
                using std::swap;
    ...
    public:
        ...
        //
        explicit ThreadpoolTimer(_In_opt_ const PTP_CALLBACK_ENVIRON _ptp_env = nullptr) NOEXCEPT :
            tp_environment(_ptp_env)
        {
            if (!::InitializeCriticalSectionEx(&timer_lock, 4000, 0)) {
                auto gle = ::GetLastError();
                ntl::AlwaysFatalCondition(L"InitializeCriticalSectionEx failed unexpectedly: %u\n", gle);
            }
        }
        ...
        void schedule_singleton(std::function<void(void)> _function, long long _millisecond_offset)
        {
            ...
            this->insert_callback_info(
                ::ntl::details::ThreadpoolTimerCallbackInfo(
                    std::move(_function), 
                    _millisecond_offset));
        }
    ...
    private:
        ...
        std::vector<::ntl::details::ThreadpoolTimerCallbackInfo> callback_objects;
        bool exiting = false;

        PTP_TIMER create_tp()
        {
            PTP_TIMER ptp_timer = ::CreateThreadpoolTimer(ThreadpoolTimerCallback, this, this->tp_environment);
            if (nullptr == ptp_timer) {
                throw ntl::Exception(::GetLastError(), L"CreateThreadpoolTimer", L"ntl::ThreadpoolTimer", false);
            }
            return ptp_timer;
        }
        //
        ...
        //
        void insert_callback_info(::ntl::details::ThreadpoolTimerCallbackInfo&& _new_request)
        {
            ntl::AutoReleaseCriticalSection lock_timer(&this->timer_lock);
            ...
            auto unused_callback = std::find_if(
                ...
                [] (const ::ntl::details::ThreadpoolTimerCallbackInfo& _info) {
            ...
            if (unused_callback == std::end(this->callback_objects)) {
                ...
                ScopeGuard(removeCallbackObjectOnFailure, { this->callback_objects.pop_back(); });

                PTP_TIMER temp_timer = this->create_tp();
                // ensure the timer is closed (is exception safe) with a scope guard
                ScopeGuard(deleteTemporaryTimerOnFailure, { ::CloseThreadpoolTimer(temp_timer); });
        ...
        static void CALLBACK ThreadpoolTimerCallback(
          ...
          PTP_TIMER _timer)
        {
            ThreadpoolTimer* this_ptr = reinterpret_cast<ThreadpoolTimer*>(_context);
            // save off the functor to invoke outside the lock
            std::function<void(void)> functor;

            // scope for the CS lock
            {
                ntl::AutoReleaseCriticalSection lock_timer(&this_ptr->timer_lock);
                ...
                ntl::FatalCondition(
                    ...
                    L"ThreadpoolTimer - failed to find the PTP_TIMER (%p) which initiated this timer callback (ntl::ThreadpoolTimer %p)",
                    _timer, this_ptr);
                ...
                if (0 == this_ptr->callback_objects[iterator_offset].reoccuring_period) {
                    ...
                    ntl::details::ThreadpoolTimerCallbackInfo empty;
